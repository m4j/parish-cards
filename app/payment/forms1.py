...
class PaymentMixin:
    ...
    def load_from(self, payment_sub):
        ...
        self.identifier.data = payment_sub.identifier
...
class PaymentSubDuesMixin(PaymentMixin, PaymentRangeMixin):

    def __init__(self, card, *args, **kwargs):
        super(PaymentSubDuesMixin, self).__init__(*args, **kwargs)
...
class PaymentSubDuesForm(PaymentSubDuesMixin, FlaskForm):
    submit_btn = SubmitField('Submit')

class PaymentSubProsphoraMixin(PaymentMixin, PaymentRangeMixin):
    quantity = IntegerField('Quantity', validators=[DataRequired()])
    with_twelve_feasts = BooleanField('12 Feasts')

    def __init__(self, prosphora, *args, **kwargs):
        super(PaymentSubProsphoraMixin, self).__init__(*args, **kwargs)
...
class PaymentSubProsphoraForm(PaymentSubProsphoraMixin, FlaskForm):
    submit_btn = SubmitField('Submit')

class PaymentSubMiscMixin(PaymentMixin):
    category = SelectField('Category', validators=[DataRequired()])

    def __init__(self, *args, **kwargs):
        super(PaymentSubMiscMixin, self).__init__(*args, **kwargs)
