...
\BLOCK{set ns = namespace(payor = "",method="",identifier="",section="")}
\begin{longtable}[l]{p{2.85in} >{\raggedright}p{2.3in} l l l l}
\BLOCK{for row in data}
    \BLOCK{set payor = row["Payor"]}
    \BLOCK{set method = row["Method"]}
    \BLOCK{set section = row["Section"]}
    \BLOCK{if section != ns.section}
        \BLOCK{set ns.section = section}
        \\
    \BLOCK{endif}
    ...
    \BLOCK{endif}
    &
    {\raggedright \VAR{row["Purpose"]}} & 
    \parbox{0.38in}{\raggedleft 
        \BLOCK{if method == ""}
            ~
        \BLOCK{else}
            \VAR{method}
